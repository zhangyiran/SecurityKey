#include "com_yiran_securitykey_NativeUtils.h"
#include <stdio.h>
#include <string.h>
#include <android/log.h>

#define  LOG_TAG    "nativekeyutils"
#define  LOGI(...)  __android_log_print(ANDROID_LOG_INFO,LOG_TAG,__VA_ARGS__)

const char* SIGN = "3425012977";
...
JNIEXPORT jstring JNICALL Java_com_yiran_securitykey_NativeUtils_getKey
        (JNIEnv *env, jobject thizz, jobject thiz){
    jstring imei = getAppendedString(env, thizz, getDeviceID(env, thizz, thiz),getSerialNumber(env, thizz, thiz));

    jstring sign = getSignatures(env, thizz, thiz);
    char* signstr;
    signstr = (char*)env->GetStringUTFChars(sign,0);
    if (strcmp(SIGN,signstr) == 0) {
